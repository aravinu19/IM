                    ...
                    mDatabase.child(firebaseAuth.getCurrentUser().getUid()).addValueEventListener(new ValueEventListener() {
                        ...
                        public void onDataChange(DataSnapshot dataSnapshot) {
                            ...
                            if (URLUtil.isValidUrl(imageUrl))
                                Picasso.with(getApplicationContext()).load(Uri.parse(imageUrl)).into(imageProfile);
                        }
    ...
    public void uploadImage(final Uri fileUri) {
        ...
        currentUserDB.child("image").addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                filepath.putFile(fileUri).addOnSuccessListener(AccountDetails.this, new OnSuccessListener<UploadTask.TaskSnapshot>() {
                    @Override
                    public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                        progressDialog.dismiss();
                        Uri downloadUri = taskSnapshot.getDownloadUrl();
                        Toast.makeText(getApplicationContext(), "Finished", Toast.LENGTH_SHORT).show();
                        Picasso.with(getApplicationContext()).load(fileUri).fit().centerCrop().into(imageProfile);
                        DatabaseReference currentUserDB = mDatabase.child(mAuth.getCurrentUser().getUid());
                        currentUserDB.child("image").setValue(downloadUri.toString());
                    }
                }).addOnFailureListener(AccountDetails.this, new OnFailureListener() {
                    @Override
